...
abc_seurat_cl08_str_dropSubc057_0p8 <- readRDS(
    file.path(clust_dir, "abc_seurat_cl08_str_dropSubc057_0p8.rds")
)

hsdir <- file.path(clust_dir, "hotspot")
abc_hs_modules <- read.csv(file.path(hsdir, "modules.csv"))
...
for (i in seq_len(max(abc_hs_modules$Module))) {
    GeneModuleScorePlot(
        abc_seurat_cl08_str_dropSubc057_0p8,
        subset(abc_hs_modules, Module == i)$X
    ) + 
        labs(
