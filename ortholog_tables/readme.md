The final tables `marm_human_orthologs.csv` and `marm_mouse_orthologs.csv` are included in this repo and so nothing here has to be rerun.
...
For reciprocal hotspot projections (which include the developmental datasets of mouse, pig, and ferret), another ortholog table was download, this time from ENSEMBL v112 using human (GRCh38.p14) as the initial reference organism (`ens_112_ortho_human_ferret_pig_mouse_marmoset.csv`). 
