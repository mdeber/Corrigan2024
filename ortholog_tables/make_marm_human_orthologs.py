...
path_out = Path(path_ortho_tables, "marm_human_orthologs.csv")
...
df_ortho = df_ortho[
    (df_ortho['Human orthology confidence [0 low, 1 high]'] == 1)
]
...
df_ortho = df_ortho.sort_values(
    by = ['Gene name', 'Gene stable ID version', '%id. query gene identical to target Human gene'],
    ascending = [True, False, False]
)
df_ortho = df_ortho.set_index('Gene name')
df_ortho = df_ortho.loc[[not i for i in df_ortho.index.duplicated(keep='first')], ]
