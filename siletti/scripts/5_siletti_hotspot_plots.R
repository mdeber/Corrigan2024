...
siletti_neur_roi_sclust_dropAllMeis2_0p8 <- readRDS(
    file.path(clust_dir, "siletti_neur_roi_sclust_dropAllMeis2_0p8.rds")
)
hsdir <- file.path(clust_dir, "hotspot")
siletti_hs_modules <- read.csv(file.path(hsdir, "modules.csv"))
...
for (i in seq_len(max(siletti_hs_modules$Module))) {
    GeneModuleScorePlot(
        siletti_neur_roi_sclust_dropAllMeis2_0p8,
        subset(siletti_hs_modules, Module == i)$X
    ) + 
        labs(
